...
A health insurance claims analytics pipeline built on the Hadoop ecosystem: **HDFS → Spark (ETL) → Hive (SQL analytics)**, with a migration path to a cloud warehouse (Snowflake) that's been run and verified end-to-end against a live account, not just sketched as SQL.
...
- **Tested and verified against a live account:** the Snowflake migration path (`snowflake/setup.sql`, `snowflake/load.sql`) — all 150,000 curated rows loaded and cross-verified against Spark's output and an independent pandas recomputation. See the [Snowflake section](#bonus-cloud-migration-path-snowflake--verified-against-a-live-account) below and `docs/resume_bullet.md`.
...
├── snowflake/
│   ├── setup.sql                          # Create claims_db.warehouse + stage/table
│   └── load.sql                            # Per-partition PUT + COPY INTO (verified)
├── docs/
│   └── resume_bullet.md                     # Cross-verified metrics writeup
└── curated/                                 # Local smoke-test output (Parquet + CSV)
```

## Bonus: cloud migration path (Snowflake) — verified against a live account
...
Unlike the rest of the "bonus" framing below, this migration path has actually been run end-to-end: `snowflake/setup.sql` and `snowflake/load.sql` were executed against a live Snowflake account and loaded all 150,000 curated claim rows via key-pair-authenticated `snowsql`. Two details the illustrative version glossed over, discovered only by running it for real:

- **Partition collisions on a flat stage.** Spark reuses the same job UUID (and therefore the same part filenames) across every `service_month=YYYY-MM` directory, so staging the whole curated tree at once silently overwrites files. `load.sql` stages each partition into its own stage subfolder instead.
- **`service_month` isn't in the parquet payload.** It's a Hive partition column, so it only exists in the file path. `load.sql` recovers it from `METADATA$FILENAME` at `COPY INTO` time instead of assuming it's a selectable column.

Run it yourself:
```bash
snowsql -a <account> -u <user> --private-key-path <key.p8> -f snowflake/setup.sql
snowsql -a <account> -u <user> --private-key-path <key.p8> -f snowflake/load.sql
```

The resulting `CLAIMS_DB.WAREHOUSE.CLAIMS` table was cross-verified against Spark's own curated output and an independent pandas recomputation — all three agreed to the decimal on PMPM, denial rate, and readmission rate. See `docs/resume_bullet.md` for the full verification writeup, including two real bugs (a readmission-rate denominator error and a case-sensitivity bug on `claim_status`) that were caught and fixed in the process.

Note what's still *not* verified: the HDFS/Hive path in the architecture diagram above, and getting the curated Parquet from local disk to S3/GCS/Azure Blob for the stage (`load.sql` here loads straight from local disk via `PUT`, sidestepping that step — a real migration still needs `distcp`/`hadoop fs -cp` plus a storage integration).
